...
def create_absolute_attribute_predicate(cnn_model):
    ...
    class AbsoluteAttributePredicate(nn.Module):
        ...
        def forward(self, x, class_label):
            ...
            # Get CNN logits for all classes - only pass x, not class_label
            all_logits = self.cnn_model(x)
            
            # Ensure logits are at least 2D [batch_size, n_classes]
            if all_logits.dim() == 1:
                all_logits = all_logits.unsqueeze(0)
            
            # class_label should be tensor, ensure proper shape for indexing
            if class_label.dim() > 1:
                class_label = class_label.squeeze()
            if class_label.dim() == 0:  # Handle scalar case
                class_label = class_label.unsqueeze(0)
                
            # Get class indices
            class_indices = class_label.long()
            
            # Use gather to select logits for the specified classes
            # all_logits: [batch_size, n_classes]
            # class_indices: [batch_size]
            selected_logits = torch.gather(all_logits, 1, class_indices.unsqueeze(1)).squeeze(1)
